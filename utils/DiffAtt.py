...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, dropout=0.1, max_len=17520):
        ...
        # div_term将位置映射到不同频率的正弦和余弦函数
        div_term = torch.exp(torch.arange(0, d_model, 2).float() * (-math.log(10000.0) / d_model))

        # 奇数列使用sin函数,偶数列使用cos函数
...
class ABI_MultiHeadAttention(nn.Module):
    ...
    def forward(self, q_action, k_backgroud, v_action, v_background):
        ...
        # (B, S, D) -proj-> (B, S, D_new) -split-> (B, S, H, W) -trans-> (B, H, S, W)
        Q = self.W_Q(q_action).view(batch_size, -1, n_heads, d_k).transpose(1,
                                                                            2)  # Q: [batch_size, n_heads, len_q, d_k]
        K = self.W_K(k_backgroud).view(batch_size, -1, n_heads, d_k).transpose(1,
                                                                               2)  # K: [batch_size, n_heads, len_k, d_k]
        V_A = self.W_V_A(v_action).view(batch_size, -1, n_heads, d_v).transpose(1,
                                                                                2)  # V: [batch_size, n_heads, len_v(=len_k), d_v]
        V_B = self.W_V_B(v_background).view(batch_size, -1, n_heads, d_v).transpose(1,
                                                                                    2)  # V: [batch_size, n_heads, len_v(=len_k), d_v]

        # context: [batch_size, n_heads, len_q, d_v], attn: [batch_size, n_heads, len_q, len_k]
        context_a, context_b, attn = ABI_ScaledDotProductAttention()(Q, K, V_A, V_B)
        context_a = context_a.transpose(1, 2).reshape(batch_size, -1,
                                                      n_heads * d_v)  # context: [batch_size, len_q, n_heads * d_v]
        context_b = context_b.transpose(1, 2).reshape(batch_size, -1,
                                                      n_heads * d_v)  # context: [batch_size, len_q, n_heads * d_v]
...
class Encoder(nn.Module):
    ...
    def forward(self, action_feat, background_feat):
        ...
        background_feat = self.pos_emb_background(background_feat.transpose(0, 1)).transpose(0,
                                                                                             1)  # [batch_size, src_len, d_model]
...
class Transformer(nn.Module):
    def __init__(self):
        ...
        self.query = nn.Embedding(timestep, d_model)
        self.fusion_conv = nn.Conv1d(d_model * 2, d_model, kernel_size=3, padding=1, )
    ...
    def forward(self, inputs):
        ...
        action_feat_out, background_feat_out, _ = self.encoder(action_feat,
                                                               background_feat)  # [batch_size, src_len, d_model]

        output = torch.cat((action_feat_out, background_feat_out), dim=2)
        output = self.fusion_conv(output.permute(0, 2, 1)).permute(0, 2, 1)
        return action_feat_out


def get_frame_abi_model():
    global src_len, tgt_len, d_model, d_ff, d_k, n_layers, n_heads, d_v, timestep
    timestep = 17520 // 10
    d_model = 512
    d_ff = 1024
    d_k = d_v = 64
...
def DiffAtt():
    # 输入数据维度(b,c,t)    torch.randn(8,64,timestep)
    # 输出是(b,t,c)       torch.Size([8, 1752, 128])
    return get_frame_abi_model()
